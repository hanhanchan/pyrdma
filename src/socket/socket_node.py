...
from src.common.buffer_attr import deserialize
...
class SocketNode:
    def __init__(self, name, options=c.OPTIONS):
        ...
        self.buffer_attr, self.buffer_basic_attr, self.buffer_key_attr = self.init_buffer_attr(self.file_mr,
                                                                                               c.FILE_SIZE)
    ...
    def init_buffer_attr(self, mr: MR, buffer_len=c.BUFFER_SIZE):
        ...
        return BufferAttr(mr.buf, buffer_len, mr.lkey, mr.rkey, str(self.gid), self.qp.qp_num), BufferBasic(
            str(self.gid), self.qp.qp_num), BufferKey(mr.buf, buffer_len, mr.lkey, mr.rkey)
    ...
    def p_trans_send(self, server_metda, content):
        self.poll_cq()  # post recv
        file_stream_pool = []
        msg = self.recv_mr.read(c.BUFFER_SIZE, 0)
        if utils.check_msg(msg, m.FILE_BEGIN_MSG):
